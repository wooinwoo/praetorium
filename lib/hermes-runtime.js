...
import {
  CODEX_COMPATIBILITY, PINNED_HERMES_VERSION, WslRuntime,
  runtimeReadinessError, supportsCodexVersion,
} from './wsl-runtime.js';
...
const WINDOWS_APP_CONTROL_EXIT_CODE = 0xc0e90002;
...
function hermesRootFromExecutable(executable) {
  ...
  throw new Error('Hermes executable must be inside a managed hermes-agent virtual environment or hermes-agent/bin');
}

function defaultHermesExecutable({
  localAppData = process.env.LOCALAPPDATA || join(homedir(), 'AppData', 'Local'),
  fileExists = existsSync,
} = {}) {
  const agentRoot = join(localAppData, 'hermes', 'hermes-agent');
  const candidates = [
    join(agentRoot, 'praetorium-venv', 'Scripts', 'hermes.exe'),
    join(agentRoot, 'venv', 'Scripts', 'hermes.exe'),
    join(agentRoot, 'bin', 'hermes.exe'),
  ];
  return candidates.find(candidate => fileExists(candidate)) || candidates.at(-1);
}

function hermesProcessError(code, stderr = '') {
  if ((Number(code) >>> 0) === WINDOWS_APP_CONTROL_EXIT_CODE) {
    return Object.assign(new Error(
      'Windows Smart App Control blocked the Hermes Python runtime. Reinstall the local runtime from 환경 관리 without disabling Windows security.',
    ), { code: 'HERMES_WINDOWS_APP_CONTROL_BLOCKED' });
  }
  return new Error(String(stderr || '').trim() || `Hermes exited with code ${code}`);
}
...
export class HermesRuntime {
  constructor({
    hermesBin = process.env.HERMES_BIN || defaultHermesExecutable(),
    spawnImpl = spawn,
    wslRuntime = null,
  ...
  async run(args, { cwd, board, input = null, timeoutMs = this.timeoutMs, onOutput = null, directorMode = false, target = null } = {}) {
    ...
    let spawnEnv = {
      ...localOnlyEnv(),
      // Hermes' kanban dispatcher spawns each Worker through HERMES_BIN.
      // Pin it to the same trusted executable that launched the Director;
      // otherwise a stale uv-managed Hermes earlier on PATH can reintroduce
      // an unsigned Python runtime under Windows Smart App Control.
      HERMES_BIN: this.hermesBin,
      ...targetEnv,
      ...(board ? {
    ...
    return new Promise((resolve, reject) => {
      ...
      const finish = (code, signal) => {
        ...
        else reject(Object.assign(hermesProcessError(code, stderr), { result }));
      };
  ...
  taskLogPath({ board, taskId, target }) {
    assertSafeId(board, 'board');
    assertSafeId(taskId, 'task');
    if (runtimeTarget(target).kind !== 'windows') return null;
    const root = hermesRootFromExecutable(this.hermesBin);
    const logsRoot = board === 'default'
      ? win32.join(root, 'kanban', 'logs')
      : win32.join(root, 'kanban', 'boards', board, 'logs');
    const candidate = win32.join(logsRoot, `${taskId}.log`);
    if (win32.dirname(candidate) !== logsRoot) throw new Error('Worker log path escaped its board');
    return candidate;
  }
  ...
  async scheduleTask({ profile, board, cwd, taskId, reason = 'Paused by Owner', target }) {
    assertSafeId(taskId, 'task');
    return this.kanban({
      profile, board, cwd,
      args: ['schedule', taskId, String(reason).slice(0, 2000)], target,
    });
  }
  ...
  async _describeTargetsFresh({ force = false } = {}) {
    ...
    const windowsProbe = Promise.allSettled([
      ...
      executableVersion(this.spawnImpl, process.env.CODEX_BIN || 'codex', ['app-server', '--help']),
    ]);
    const wslProbe = this.wslRuntime.listTargets({ force })
      .then(targets => ({ targets, error: null }))
      .catch(error => ({ targets: [], error: error.message }));
    const [[hermesCheck, codexCheck, codexAuthCheck, codexAppServerCheck], wslResult] = await Promise.all([windowsProbe, wslProbe]);
    const hermesVersion = hermesCheck.status === 'fulfilled' ? (hermesCheck.value.stdout || hermesCheck.value.stderr) : null;
    const hermesError = hermesCheck.status === 'rejected' ? hermesCheck.reason?.message || 'Hermes version check failed.' : null;
    const codexVersion = codexCheck.status === 'fulfilled' ? codexCheck.value : null;
    const hermesPinned = Boolean(hermesVersion?.includes(PINNED_HERMES_VERSION));
    const codexCompatible = supportsCodexVersion(codexVersion);
    const profilesReady = PROFILE_CATALOG.every(profile => profiles.includes(profile.id));
    const windows = {
      id: 'windows', kind: 'windows', distro: null, label: 'Windows', checkedAt: Date.now(),
      ready: Boolean(hermesPinned && codexCompatible && codexAppServerCheck.status === 'fulfilled'
        && codexAuthCheck.status === 'fulfilled' && profilesReady),
      hermes: { installed: Boolean(hermesVersion), path: this.hermesBin, version: hermesVersion, pinned: hermesPinned, error: hermesError },
      codex: {
        installed: Boolean(codexVersion), path: process.env.CODEX_BIN || 'codex', version: codexVersion,
        compatible: codexCompatible, compatibility: CODEX_COMPATIBILITY,
        appServer: codexAppServerCheck.status === 'fulfilled', authenticated: codexAuthCheck.status === 'fulfilled',
      },
      profiles, profilesReady,
      error: null,
    };
    windows.error = runtimeReadinessError(windows);
    return {
...
export const _test = {
  ...
  hermesRootFromExecutable, defaultHermesExecutable, hermesProcessError, runtimeTarget,
};
