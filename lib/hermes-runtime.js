...
export class HermesRuntime {
  ...
  candidateSnapshot({ cwd, target }) {
    const selectedTarget = runtimeTarget(target);
    if (selectedTarget.kind === 'wsl') {
      return this.wslRuntime.candidateSnapshot({ distro: selectedTarget.distro, path: cwd });
    }
    return snapshotWindowsCandidate({ cwd, spawnImpl: this.spawnImpl });
  }
