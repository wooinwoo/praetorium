...
export function projectKey(project) {
  ...
  const distro = String(project?.distro || '').trim().toLowerCase();
  const path = String(project?.path || '');
  return `${runtime}:${distro}:${runtime === 'wsl' ? path : path.toLowerCase()}`;
}
...
export function addProject(input) {
  ...
  const pathKey = projectKey(project);
  if (state.projects.some(item => projectKey(item) === pathKey)) throw new Error('이미 배정된 프로젝트입니다.');
...
export const _test = { slug, cleanProject, validDirectory, projectRuntime, projectKey, projectSlot, projectIdentity, assignProjectSlots };
