...
const SUBMODULE_TIMEOUT_MS = 5000;
...
async function gitCandidateState(root, spawnImpl) {
  const [head, listed, status, index] = await Promise.all([
  ...
  const hasGitlink = index.stdout.toString('utf8').split('\0').some(record => record.startsWith('160000 '));
  let submoduleBytes = Buffer.alloc(0);
  if (hasGitlink) {
    try {
      await lstat(resolve(root, '.gitmodules'));
      const submodules = await capture(
        spawnImpl, 'git', ['-C', root, 'submodule', 'status', '--recursive'],
        { timeoutMs: SUBMODULE_TIMEOUT_MS },
      );
      submoduleBytes = submodules.stdout;
    } catch (error) {
      if (error.code !== 'ENOENT') throw error;
      submoduleBytes = Buffer.from('gitlinks-without-gitmodules');
    }
  }
  return {
...
export const _test = {
  ...
  MAX_HASH_BYTES, MAX_FILES, HASH_DEADLINE_MS, SUBMODULE_TIMEOUT_MS,
};
