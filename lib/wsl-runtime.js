...
export const CODEX_COMPATIBILITY = `>=${MINIMUM_CODEX_VERSION} <1.0.0`;
...
function parseWslVerbose(buffer) {
  return decode(buffer).split(/\r?\n/).flatMap(raw => {
    const match = raw.match(/^\s*(\*)?\s*(.*?)\s{2,}(.+?)\s{2,}(\d+)\s*$/u);
    if (!match) return [];
    const name = match[2].trim();
    if (!name || !SAFE_DISTRO.test(name)) return [];
    return [{
      name,
...
function parseCodexVersion(value) {
  const match = String(value || '').match(/^\s*codex-cli\s+(\d+)\.(\d+)\.(\d+)\s*$/m);
  return match ? match.slice(1, 4).map(Number) : null;
}

export function supportsCodexVersion(value) {
  const current = parseCodexVersion(value);
  const minimum = MINIMUM_CODEX_VERSION.split('.').map(Number);
  if (!current || current[0] !== minimum[0]) return false;
...
export function runtimeReadinessError(status) {
  if (status.hermes?.error) return status.hermes.error;
  if (!status.hermes?.installed) return 'Hermes를 찾지 못했습니다.';
  if (!status.hermes?.pinned) return `${PINNED_HERMES_VERSION}이 필요합니다. 현재 ${status.hermes.version || '버전 확인 실패'}.`;
  if (!status.codex?.installed) return 'Codex CLI를 찾지 못했습니다.';
  if (!status.codex?.compatible) return `Codex CLI 버전 ${CODEX_COMPATIBILITY}가 필요합니다. 현재 ${status.codex.version || '버전 확인 실패'}.`;
  if (!status.codex?.appServer) return '설치된 Codex CLI가 app-server 명령을 지원하지 않습니다.';
  if (!status.codex?.authenticated) return '이 실행 환경에서 Codex 로그인이 필요합니다.';
  if (!status.profilesReady) return `Praetorium 역할 프로필이 ${status.profiles?.length || 0} / ${PROFILE_CATALOG.length}개 설치되어 있습니다.`;
  return null;
}
...
export class WslRuntime {
  ...
  async listDistributions() {
    ...
    try {
      const result = await this._run(['--list', '--verbose']);
      const distributions = parseWslVerbose(result.stdout);
      if (distributions.length) return distributions;
    } catch (error) {
      if (error.code === 'ENOENT') return [];
      // Older WSL builds may not support --verbose; the quiet list still gives
      // project discovery a usable, fail-closed target list.
    }
    try {
      const result = await this._run(['--list', '--quiet']);
      return parseWslList(result.stdout).map(name => ({
        name, state: null, version: null, default: false,
        system: /^docker-desktop(?:-data)?$/i.test(name),
      }));
    } catch (error) {
      if (error.code === 'ENOENT') return [];
  ...
  async inspect(distro, { force = false, metadata = null } = {}) {
    const name = assertDistro(distro);
    const cached = this.cache.get(name);
    if (!force && cached && Date.now() - cached.checkedAt < 15000) return { ...cached };
    const distribution = metadata || (await this.listDistributions()).find(item => item.name === name) || {};
    const status = {
      id: `wsl:${name}`,
      kind: 'wsl',
      distro: name,
      label: `WSL · ${name}`,
      state: distribution.state || null,
      wslVersion: Number.isInteger(distribution.version) ? distribution.version : null,
      default: Boolean(distribution.default),
      system: Boolean(distribution.system),
      ready: false,
      checkedAt: Date.now(),
      home: null,
      user: null,
      ...
      codex: {
        installed: false, path: null, version: null, compatible: false,
        compatibility: CODEX_COMPATIBILITY, appServer: false, authenticated: false,
      },
      profiles: [],
      profilesReady: false,
      error: null,
    ...
    if (status.system) {
      status.error = 'Praetorium 실행 대상이 아닌 WSL 시스템 배포판입니다.';
      this.cache.set(name, status);
      return { ...status };
    }
    if (status.wslVersion !== 2) {
      status.error = status.wslVersion === 1
        ? 'WSL1 배포판입니다. Praetorium 프로젝트 실행에는 WSL2가 필요합니다.'
        : '배포판의 WSL2 버전을 확인할 수 없습니다.';
      this.cache.set(name, status);
      return { ...status };
    }
    try {
      ...
      const versionChecks = await Promise.allSettled([
        ...
        codexPath ? this._run(['--distribution', name, '--exec', codexPath, 'app-server', '--help']) : Promise.reject(new Error('Codex not found')),
      ]);
      ...
      if (versionChecks[1].status === 'fulfilled') {
        ...
        status.codex.compatible = supportsCodexVersion(status.codex.version);
      }
      status.codex.authenticated = versionChecks[2].status === 'fulfilled';
      status.codex.appServer = versionChecks[3].status === 'fulfilled';
      if (home) {
        try {
      ...
      status.profilesReady = PROFILE_CATALOG.every(profile => status.profiles.includes(profile.id));
      status.ready = status.hermes.pinned && status.codex.compatible && status.codex.appServer
        && status.codex.authenticated && status.profilesReady;
      status.error = runtimeReadinessError(status);
    } catch (error) {
  ...
  async listTargets({ force = false } = {}) {
    ...
    return Promise.all(distros.map(item => this.inspect(item.name, { force, metadata: item })));
  }

  async assertProjectDistribution(distro) {
    const name = assertDistro(distro);
    const distribution = (await this.listDistributions()).find(item => item.name === name);
    if (!distribution) throw new Error('선택한 WSL 배포판을 찾을 수 없습니다.');
    if (distribution.system) throw new Error('Praetorium 프로젝트에는 WSL 시스템 배포판을 사용할 수 없습니다.');
    if (distribution.version !== 2) {
      throw new Error(distribution.version === 1
        ? 'WSL1 배포판입니다. Praetorium 프로젝트 실행에는 WSL2가 필요합니다.'
        : '배포판의 WSL2 버전을 확인할 수 없습니다.');
    }
    return distribution;
  ...
  async validateProject({ distro, path }) {
    ...
    await this.assertProjectDistribution(name);
    try {
  ...
  async discoverProjects({ distro, root }) {
    const name = assertDistro(distro);
    const distribution = await this.assertProjectDistribution(name);
    const home = (await this.inspect(name, { metadata: distribution })).home;
  ...
  async launch({ distro, cwd, args, env = {}, board = null }) {
    ...
    if (!target.ready) throw new Error(`${target.label} 런타임 준비 실패: ${target.error || '환경 관리에서 진단 결과를 확인하세요.'}`);
    const cleanEnv = {
      ...env,
      HOME: target.home,
      USER: target.user,
...
export const _test = { assertDistro, decode, parseCodexVersion, parseWslList, parseWslVerbose };
