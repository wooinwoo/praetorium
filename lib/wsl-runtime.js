...
export class WslRuntime {
  ...
  async candidateSnapshot({ distro, path }) {
    const name = assertDistro(distro);
    ...
    const script = [
      ...
      '      if test -L "$file"; then printf "symlink\\0"; readlink -- "$file"',
      '      elif test -f "$file"; then before="$(stat -c "%s:%y" -- "$file")"; size="${before%%:*}"; total=$((total + size)); test "$total" -le "$max_bytes" || { printf "candidate-too-large" >&2; exit 43; }; printf "size\\0%s\\0" "$size"; sha256sum -- "$file" | cut -d " " -f 1; after="$(stat -c "%s:%y" -- "$file")"; test "$before" = "$after" || { printf "candidate-changed" >&2; exit 44; }',
      ...
      'else',
      '  fallback_manifest() { find . \\( -type f -o -type l \\) -not -path "./.git/*" -not -path "./node_modules/*" -print0 | sort -z | sha256sum | cut -d " " -f 1; }',
      '  before_metadata="$(fallback_manifest)"',
      '  dirty=1',
      '  digest="$({ total=0; count=0; find . \\( -type f -o -type l \\) -not -path "./.git/*" -not -path "./node_modules/*" -print0 | sort -z | while IFS= read -r -d "" file; do count=$((count + 1)); test "$count" -le 20000 || exit 42; printf "path\\0%s\\0" "$file"; if test -L "$file"; then printf "symlink\\0"; readlink -- "$file"; else before="$(stat -c "%s:%y" -- "$file")"; size="${before%%:*}"; total=$((total + size)); test "$total" -le "$max_bytes" || exit 43; printf "size\\0%s\\0" "$size"; sha256sum -- "$file" | cut -d " " -f 1; after="$(stat -c "%s:%y" -- "$file")"; test "$before" = "$after" || exit 44; fi; done; } | sha256sum | cut -d " " -f 1)"',
      '  after_metadata="$(fallback_manifest)"',
      '  test "$before_metadata" = "$after_metadata" || { printf "candidate-metadata-changed" >&2; exit 45; }',
      'fi',
      'printf "%s\\nsha256:%s\\n%s\\n" "$revision" "$digest" "$dirty"',
    ].join('\n');
    const result = await this._run([
      '--distribution', name, '--exec', '/bin/bash', '-c', script, 'praetorium-snapshot', root,
    ], { timeoutMs: 60000 });
    const [revision, digest, dirty] = decode(result.stdout, 'utf8').split(/\r?\n/);
    ...
    return {
      ...
      dirty: dirty === '1', fileCount: null, observedAt: new Date().toISOString(),
    };
  }
