...
export class DirectorService extends EventEmitter {
  ...
  _finishGoal(goal, status, report, details = null) {
    ...
    if (director) this._scheduleQueuedPromotion(director.id);
  }
  ...
  _assertAuthorityActionPrerequisites(goal, actions = [], gateAudit = null) {
    ...
    if (action.effect === 'skill_activation' && goal.workflowId !== 'skill-development') {
      throw new Error('Authority action rejected: skill activation requires the skill-development workflow.');
    }
    const policy = workflowPolicyById(goal.workflowId);
    ...
    try {
        ...
        try {
          ...
          if (run.analysis) {
            try {
              ...
              const combinedPlan = validateDirectorControl(combinedParsed.control, { requiredMode: run.resolvedMode });
      ...
      if (!plan) for (let attempt = 1; attempt <= run.maxAttempts; attempt += 1) {
        ...
        try {
          ...
          if (plan.state === 'executing') this._assertAuthorityActionPrerequisites(goal, plan.actions, null);
          break;
        } catch (error) {
          lastPlanningError = error.message;
          const retryable = error.code === 'HERMES_TIMEOUT'
            || /Director (?:control|did not return)|Execution requests must be delegated|does not match validated analysis|Authority action rejected/i.test(error.message);
          if (!retryable || attempt >= run.maxAttempts) throw error;
          director.sessionId = null;
    ...
    try {
      ...
      for (let attempt = 1; attempt <= run.maxAttempts; attempt += 1) {
        ...
        try {
          ...
          const retryable = error.code === 'HERMES_TIMEOUT'
            || /Director (?:control|did not return)|Goal completion rejected|workflow is immutable|Execution requests must be delegated|Authority action rejected/i.test(error.message);
          if (!retryable || attempt >= run.maxAttempts) throw error;
  ...
  async tickDirector(directorId, requestedMax = null) {
    ...
    try {
      try {
        ...
        const max = controls.unresolved ? 0 : Math.min(requested, available, writerSafety.cap);
        const safetyStatus = {
          ...writerSafety,
        ...
        if (shouldDispatch) {
          ...
          observedTasks = mutatedBoard
            ? await this._refreshBoard(director, { force: true }).catch(() => tasks)
            : tasks;
          dispatchResult = {
            ready, running, globalRunning, available, allocated: max,
            spawned, dispatch: result.json, tasks: observedTasks,
            reconciliationDue, writerSafety: safetyStatus,
          };
        } else {
          observedTasks = tasks;
          dispatchResult = {
            ...
            reconciliationDue: false, dispatchSkipped: true, writerSafety: safetyStatus,
          };
        }
      } finally {
