...
function declaredCandidatePaths(goal) {
  ...
  for (const record of goal?.taskRecords || []) {
    if (WORKER_PROFILES[record?.profile]?.kind !== 'write') continue;
...
function interventionTransport(id, message) {
  ...
  const body = String(message || '').trim().slice(0, Math.max(1, MAX_INTERVENTION_TRANSPORT_CHARS - prefix.length - suffix.length));
...
export class DirectorService extends EventEmitter {
  constructor({
    ...
    boardDrainTimeoutMs = BOARD_DRAIN_TIMEOUT_MS,
  } = {}) {
    super();
  ...
  getGoalHistory(directorId, { offset = 0, limit = CONSOLE_HISTORY_PAGE_LIMIT, query = '', filter = 'all' } = {}) {
    if (!this.getDirector(directorId)) throw new Error('Director not found');
    ...
    const items = (this.state.goals || [])
      .filter(goal => goal.directorId === directorId && !isActiveGoal(goal) && goal.status !== 'queued')
  ...
  getMessageHistory(directorId, { offset = 0, limit = 20, knownIds = [] } = {}) {
    ...
    const items = this.state.runs.filter(run => run.directorId === directorId && !run.goalId).slice().reverse();
  ...
  getGoalDetails(id) {
    ...
    return {
      ...
      runs: this.state.runs.filter(run => run.goalId === goal.id).slice(-200),
    };
  }
  ...
  _boardTaskProfiles(directorId) {
    ...
    for (const goal of this.state.goals || []) {
      if (goal.directorId !== directorId) continue;
      for (const record of goal.taskRecords || []) {
  ...
  _queuedGoals(directorId) {
    return (this.state.goals || [])
      .filter(goal => goal.directorId === directorId && goal.status === 'queued')
      .sort((left, right) => {
  ...
  submitMessage(directorId, prompt, { mode = 'auto' } = {}) {
    ...
    if (resolvedMode === 'delegate') this._createGoal(director, run, objective, { queued: queueGoal });
    director.lastRunId = run.id;
  ...
  _contextualPrompt(run, recoveryNote = '', { stage = 'plan', analysis = null } = {}) {
    ...
    if (!history.length) return [
      ...
      '[CURRENT OWNER MESSAGE]', run.prompt,
    ].filter(Boolean).join('\n\n');
  ...
  async _executeChat(runId) {
    ...
    try {
      ...
      this._save();
      this.emit('run', { ...run });
      if (!goal && this._queuedGoals(director.id).length) this._scheduleQueuedPromotion(director.id);
    }
  }
  ...
  async controlGoal(directorId, goalId, action, { position = null, reason = '' } = {}) {
    const director = this.getDirector(directorId);
    const goal = this.getGoal(goalId);
    if (!director || !goal || goal.directorId !== directorId) {
  ...
  consoleSummary({ directorId = null } = {}) {
    ...
    const selectedQueued = queuedEntries.filter(entry => entry.goal.directorId === selectedDirector?.id);
    ...
    const selectedRuns = selectedDirector?.kind === 'project'
      ? this.listRuns({ projectId: selectedDirector.projectId, limit: CONSOLE_RECENT_RUN_LIMIT })
