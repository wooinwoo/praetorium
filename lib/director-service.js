...
import {
  ...
  WORKFLOWS, REVIEW_KIND_BY_PROFILE, isStructuredEvidenceApproved, requiredWorkflowStage,
  workflowById, workflowPolicyById,
...
function assertWorkflowWaveOrder(goal, actions, gateAudit = null) {
  if (!goal?.workflowId) throw new Error('Workflow phase rejected: the Goal has no selected workflow.');
  const prepared = dependencySafeActions(actions);
  const transition = requiredWorkflowStage(goal.workflowId, goalTaskEvidence(goal), {
    gateAudit,
    verificationAfterWave: goal.verificationBarrier?.afterWave,
  });
  const profiles = prepared.map(action => action.target);
  const kinds = prepared.map(action => WORKER_PROFILES[action.target]?.kind);
  const authorityActions = prepared.filter(action => ['external_mutation', 'skill_activation'].includes(action.effect));
  const reject = detail => {
    const allowed = transition.allowedProfiles.length ? transition.allowedProfiles.join(', ') : 'no Worker actions';
    throw new Error(`Workflow phase rejected: required stage is ${transition.stage} (${transition.reason}); allowed profiles: ${allowed}. ${detail}`);
  };

  if (transition.stage === 'complete') {
    if (authorityActions.length === 1 && prepared.length === 1) {
      return {
        preparedActions: prepared,
        transition: { ...transition, stage: 'authority', reason: 'verified-candidate-awaits-exact-owner-authority' },
        profiles,
      };
    }
    reject('Return complete instead of creating another Worker wave.');
  }
  if (transition.stage === 'candidate') {
    if (!prepared.length || prepared.some(action => !transition.allowedProfiles.includes(action.target))) {
      reject('Create only the workflow candidate-producing Worker wave before any review or gate.');
    }
  } else if (transition.stage === 'remediation') {
    if (!prepared.length || prepared.some(action => action.target !== 'remediator')) {
      reject('Assign the blocking findings to the separate remediator before fresh review.');
    }
  } else if (transition.stage === 'review') {
    if (kinds.some(kind => kind === 'write')) {
      reject('The current candidate exists; run fresh read-only reviews before another write or gate-only wave.');
    }
    const reviewActions = prepared.filter(action => WORKER_PROFILES[action.target]?.kind === 'review');
    if (!reviewActions.some(action => transition.missingReviewProfiles.includes(action.target))) {
      reject('The wave must include at least one missing or stale required reviewer.');
    }
    const gateActions = prepared.filter(action => WORKER_PROFILES[action.target]?.kind === 'gate');
    if (gateActions.length) {
      const includedReviewProfiles = new Set(reviewActions.map(action => action.target));
      const stillMissing = transition.missingReviewProfiles.filter(profile => !includedReviewProfiles.has(profile));
      if (stillMissing.length) {
        reject(`A quality gate cannot share this wave until every missing review is included: ${stillMissing.join(', ')}.`);
      }
      const reviewActionIds = reviewActions.map(action => action.id);
      for (const gate of gateActions) {
        const missingDependencies = reviewActionIds.filter(id => !gate.dependencies.includes(id));
        if (missingDependencies.length) {
          reject(`Quality gate ${gate.id} must depend on every review action in its wave: ${missingDependencies.join(', ')}.`);
        }
      }
    }
  } else if (transition.stage === 'gate') {
    if (!prepared.length || prepared.some(action => !transition.allowedProfiles.includes(action.target))) {
      reject('All required reviews are current; create only the final quality-gate wave.');
    }
  }

  return { preparedActions: prepared, transition, profiles };
}
...
function goalBoardIsolation(tasks, activeGoal) {
  if (!activeGoal) {
    return {
      safe: true,
      ...
      activeRunnableTaskIds: [],
    };
  }
  const activeIds = new Set(activeGoal.taskIds || []);
  const runnable = (Array.isArray(tasks) ? tasks : []).filter(task => (
    ['ready', 'todo', 'running'].includes(String(task?.status || '').toLowerCase())
  ));
  const foreign = runnable.filter(task => !activeIds.has(task?.id));
  const foreignRunnableTaskIds = foreign.map(task => task.id).filter(Boolean);
  return {
    safe: foreignRunnableTaskIds.length === 0,
    foreignRunnableTaskIds,
    foreignQueuedTaskIds: foreign
      .filter(task => ['ready', 'todo'].includes(String(task?.status || '').toLowerCase()))
      .map(task => task.id)
      .filter(Boolean),
    foreignRunningTaskIds: foreign
      .filter(task => String(task?.status || '').toLowerCase() === 'running')
      .map(task => task.id)
      .filter(Boolean),
    activeRunnableTaskIds: runnable.filter(task => activeIds.has(task?.id)).map(task => task.id).filter(Boolean),
  };
}
...
function projectOperationalSnapshot({
  ...
  const activeGoalRunningWorkers = boardFresh && activeGoal
    ...
    : activeGoal ? null : 0;
  const activeGoalWaitingWorkers = boardFresh && activeGoal
    ? tasks.filter(task => activeTaskIds.has(task?.id)
      && ['ready', 'todo', 'scheduled', 'review'].includes(normalizedStatus(task))).length
    : activeGoal ? null : 0;
  const quarantinedNonterminalWorkers = boardFresh && activeGoal
    ? tasks.filter(task => !activeTaskIds.has(task?.id) && !isTerminalTask(normalizedStatus(task))).length
    : 0;
  const rawTaskItems = activeGoal
  ...
  return {
    ...
    board: {
      ...
      activeGoalWaitingWorkers,
      quarantinedNonterminalWorkers,
    },
    sessions: {
...
function projectOperationalStatusPrompt(snapshot) {
  ...
  return [
    ...
    'board.runningWorkers and board.waitingWorkers are project-wide observability counts only. When activeGoal exists, only tasks.scope="active_goal", board.activeGoalRunningWorkers, and board.activeGoalWaitingWorkers belong to it. quarantinedNonterminalWorkers are explicitly foreign to the active Goal and MUST NOT influence its plan, evidence, dependencies, or completion.',
...
function consoleRequiredTransition(goal = {}) {
  if (!goal.workflowId) return null;
  try {
    return requiredWorkflowStage(goal.workflowId, goalTaskEvidence(goal), {
      verificationAfterWave: goal.verificationBarrier?.afterWave,
    });
  } catch {
    return null;
...
function consoleGoal(goal = {}, queuePosition = null) {
  return {
    ...
    requiredTransition: consoleRequiredTransition(goal),
...
function taskBody(goal, run, plan, action, waveIndex) {
  ...
  const workerKind = WORKER_PROFILES[action.target]?.kind;
  if (workerKind === 'review') {
    const reviewKind = REVIEW_KIND_BY_PROFILE[action.target];
    lines.push(
      '',
      '[STRUCTURED TERMINAL RECEIPT]',
      'Your final kanban_complete call MUST pass the complete review.v1 object as metadata.report.',
      `The report MUST use review_kind (never review_type) with the exact value "${reviewKind}".`,
      'Retain every key in this exact skeleton and replace placeholders with concrete values:',
      `{"schema":"review.v1","review_kind":"${reviewKind}","scope":{"project":"string","objective":"string","base_revision":null,"head_revision":"string|null","artifact_digest":"string|null","paths":["string"]},"verdict":"pass|warn|fail|inconclusive","summary":"string","checks":[{"id":"string","status":"pass|fail|not_applicable|not_verified","evidence":["string"]}],"findings":[],"coverage":{"examined":["string"],"omitted":[],"limitations":[],"assumptions":[]}}`,
      'If findings is non-empty, every finding MUST contain id, severity, confidence, category, title, claim, evidence[{path,line,detail}], impact, required_action, verification, and blocking.',
      'Set metadata.review_outcome to the exact string "approved" only for verdict "pass" or "warn"; otherwise set it to "rejected".',
      'Use exactly: kanban_complete(summary=<human summary>, metadata={"report": <the complete object above>, "review_outcome": "approved|rejected"}, artifacts=[]).',
      'Do not add alternate schema fields such as review_type, reviewer_role, evidence, or commands in place of the required checks and coverage fields.',
    );
  }
  if (workerKind === 'gate') lines.push(
    '',
    '[STRUCTURED TERMINAL RECEIPT]',
    'Your final kanban_complete call MUST pass the complete quality-gate.v1 object as metadata.report.',
    'Retain every key in this exact skeleton and replace placeholders with concrete values:',
    '{"schema":"quality-gate.v1","candidate":{"revision":"string|null","artifact_digest":"string|null"},"decision":"advance|stop|inconclusive","acceptance":[{"criterion":"verbatim Goal success criterion","status":"met|unmet|not_verified","evidence":["string"]}],"reports":[{"review_kind":"string","status":"current|missing|stale","verdict":"string|null"}],"blockers":[],"residual_risk":[],"next_action":"string"}',
    'Copy every Goal success criterion verbatim and in its original order into acceptance, and include one reports row for every required specialist review.',
    'Set metadata.review_outcome to the exact string "approved" only for decision "advance"; otherwise set it to "rejected".',
    'Use exactly: kanban_complete(summary=<human summary>, metadata={"report": <complete quality-gate.v1 object>, "review_outcome": "approved|rejected"}, artifacts=[]).',
    'Do not rename metadata.report, invent alternate field names, abbreviate the schema, or omit candidate, decision, acceptance, reports, blockers, residual_risk, or next_action.',
  );
  lines.push(
    '',
    '[PUBLIC TRACE]',
...
export class DirectorService extends EventEmitter {
  ...
  _contextualPrompt(run, recoveryNote = '', { stage = 'plan', analysis = null } = {}) {
    ...
    const controlContract = [
      ...
      'The host enforces candidate → review → remediation when findings exist → fresh review → quality gate → complete. The first wave of a workflow with a required write profile must use that candidate-producing profile; never begin with a reviewer or gate.',
  ...
  async _materializeGoalWave({ director, goal, run, plan, existingWave = null, gateAudit = null }) {
    await this._prepareRunAttachments(director, run, goal);
    let wave = existingWave;
    let preparedActions;
    let remediation;
    if (!wave) {
      preparedActions = assertWorkflowWaveOrder(goal, plan.actions, gateAudit).preparedActions;
    ...
    try {
      for (const action of wave.actions || []) {
        ...
        const created = await this.runtime.createTask({
          ...
          parents,
          // Every materialized Worker owns a durable board lifecycle, including
          // read-only reviewers and quality gates. A one-turn review can stop
          // after a PLAN/tool lookup without completing its card; Hermes goal
          // mode keeps the same Worker session alive until it calls the native
          // completion or block tool (within the bounded turn cap).
          goalMode: true,
  ...
  async _applyGoalControl({ director, goal, run, plan, publicOutput = '', gateAudit = null }) {
    ...
    if (plan.state === 'executing') {
      ...
      const materialized = await this._materializeGoalWave({ director, goal, run, plan, gateAudit });
    ...
    try {
          ...
          if (run.analysis) {
            try {
              ...
              if (combinedPlan.state === 'executing') {
                assertWorkflowWaveOrder(goal, combinedPlan.actions, null);
      ...
      if (!plan) for (let attempt = 1; attempt <= run.maxAttempts; attempt += 1) {
        ...
        try {
          ...
          if (plan.state === 'executing') {
            assertWorkflowWaveOrder(goal, plan.actions, null);
    ...
    const pending = (this.state.goals || [])
      ...
      .flatMap(goal => (goal.taskRecords || [])
        .filter(record => !isTerminalTask(record.status) && (record.pausePending || record.resumePending
          || (record.pausedByOwner && !['blocked', 'scheduled'].includes(String(byId.get(record.taskId)?.status || '').toLowerCase()))))
  ...
  async tickDirector(directorId, requestedMax = null) {
    ...
    try {
      try {
        ...
        let goalIsolation = goalBoardIsolation(tasks, activeGoalBeforeDispatch);
        let quarantinedForeignTaskIds = [];
        if (activeGoalBeforeDispatch
          && activeWaveBeforeDispatch?.status !== 'materializing'
          && goalIsolation.foreignQueuedTaskIds.length) {
          infrastructureOperation = 'foreign_task_quarantine';
          for (const taskId of goalIsolation.foreignQueuedTaskIds) {
            await this.runtime.scheduleTask({
              profile: director.profile,
              board: director.board,
              cwd: director.cwd,
              taskId,
              reason: `Not owned by active Praetorium Goal ${activeGoalBeforeDispatch.id}`,
              target: directorTarget(director),
            });
          }
          const attemptedTaskIds = [...goalIsolation.foreignQueuedTaskIds];
          infrastructureOperation = 'board_list';
          tasks = await this._refreshBoard(director, { force: true });
          goalIsolation = goalBoardIsolation(tasks, activeGoalBeforeDispatch);
          const stillQueued = attemptedTaskIds.filter(taskId => goalIsolation.foreignQueuedTaskIds.includes(taskId));
          if (stillQueued.length) {
            infrastructureOperation = 'foreign_task_quarantine';
            throw new Error(`Foreign Worker quarantine was not observed for: ${stillQueued.join(', ')}`);
          }
          quarantinedForeignTaskIds = attemptedTaskIds.filter(taskId => !goalIsolation.foreignRunningTaskIds.includes(taskId));
          if (quarantinedForeignTaskIds.length) {
            addGoalEvent(
              activeGoalBeforeDispatch,
              'recovery',
              'foreign_tasks_quarantined',
              '활성 Goal에 속하지 않은 대기 Worker 카드를 자동 일시정지했습니다.',
              { taskIds: quarantinedForeignTaskIds },
            );
            this._save();
          }
        }
        ...
        const max = controls.unresolved || dispatchHeldForGoal || !goalIsolation.safe
        ...
        const goalNeedsDispatch = Boolean(activeGoalBeforeDispatch && activeGoalBeforeDispatch.status !== 'awaiting_owner');
        ...
        if (shouldDispatch) {
          ...
          dispatchResult = {
            ...
            reconciliationDue, writerSafety: safetyStatus, goalIsolation, quarantinedForeignTaskIds,
            fairPeers, fairShare, totalCapacity, dispatchHeldForGoal,
          };
        } else {
          observedTasks = tasks;
          dispatchResult = {
            ...
            reconciliationDue: false, dispatchSkipped: true, writerSafety: safetyStatus, goalIsolation, quarantinedForeignTaskIds,
            fairPeers, fairShare, totalCapacity, dispatchHeldForGoal,
          };
        }
      } finally {
...
export const _test = {
  ...
  boardTaskProfile, boardTaskExecutionKind, boardDispatchSafety, goalBoardIsolation,
  assertWorkflowWaveOrder, projectOperationalSnapshot,
