...
import {
  ...
  inferredAuthorityEffect, validateDirectorAnalysis, validateDirectorControl,
...
function actionAuthorityEffect(action, { infer = false } = {}) {
  ...
  if (/(?:deploy|publish|release|ship|tag|upload|external|배포|게시|릴리스|출시|태그|외부)/i.test(actionText)) {
...
function consoleGoalDetailRevision(goal = {}) {
  return sha256Json({
    ...
    ownerAnswers: [goal.ownerAnswers?.length || 0, goal.ownerAnswers?.at(-1)?.at || null],
...
export class DirectorService extends EventEmitter {
  constructor({
    ...
    boardDrainTimeoutMs = BOARD_DRAIN_TIMEOUT_MS, attachmentStore = null, attachmentRoot = null,
  } = {}) {
    super();
  ...
  submitMessage(directorId, prompt, { mode = 'auto', attachments = [] } = {}) {
    ...
    if (resolvedMode !== 'delegate' && director.status === 'running') throw new Error('Director is already running');
    ...
    try {
      ...
      director.lastRunId = run.id;
      if (!queueGoal) director.status = 'running';
      this._save();
    } catch (error) {
    ...
    if (!queueGoal) queueMicrotask(() => this._executeChat(run.id));
    return {
      ...run,
  ...
  _contextualPrompt(run, recoveryNote = '', { stage = 'plan', analysis = null } = {}) {
    ...
    const controlContract = [
      ...
      '<PRAETORIUM_CONTROL>',
      '{"schema":"director-action.v1","mode":"conversation|delegate","workflow_id":"workflow-id-or-null","state":"executing|awaiting_owner|complete|blocked","requirements":["..."],"decisions":["public operational reason"],"actions":[{"id":"a1","title":"short title","target":"approved-worker-profile","effect":"read_only|workspace_write|external_mutation|skill_activation","task":"bounded worker outcome","skills":["approved-skill"],"dependencies":[],"write_scope":["repository-relative candidate path or descriptive read-only scope"],"acceptance":["observable evidence"],"wake_on":["completion|finding|failure"]}],"owner_decision":{"required":false,"question":null,"options":[],"evidence":[]}}',
      '</PRAETORIUM_CONTROL>',
    ...
    if (!history.length) return [
      liveStatus, communicationContract, catalogPrompt(), routingContext, stageContract, analysisContext, ownerAnswers, decisionLedger,
  ...
  _scheduleQueuedPromotion(directorId) {
    const timer = setTimeout(() => {
      try { this._promoteNextGoal(directorId); }
      catch (error) { if (this.listenerCount('error')) this.emit('error', error); }
    }, 0);
    timer.unref?.();
  }
  ...
  async _executeChat(runId) {
    ...
    run.status = 'running';
    run.startedAt = now();
    run.resolvedMode = run.autoDecision?.mode === 'delegate'
    ...
    try {
      ...
      this._save();
      this.emit('run', { ...run });
      if ((!goal || goal.status === 'queued') && this._queuedGoals(director.id).length) {
        this._scheduleQueuedPromotion(director.id);
      }
    }
  }
  ...
  async _assertGoalWorkersStopped(director, goal, { quiescePending = false } = {}) {
    const boardKey = this._boardKey(director);
    ...
    try {
      ...
      if (pending.length && !quiescePending) {
      ...
      for (const task of pending) {
        ...
        const record = goal.taskRecords?.find(item => item.taskId === task.id);
        if (record) {
          record.status = 'blocked';
          record.completedAt ||= now();
      ...
      if (pending.length) {
        const confirmed = await resolveMissingOwnedTasks(
          ...
          { forceIds: new Set(pending.map(task => task.id)) },
        );
        const unsafe = confirmed.filter(task => owned.has(task?.id)
          && ['running', ...dispatchableStatuses].includes(String(task?.status || '').toLowerCase()));
        if (unsafe.length) {
  ...
  async controlGoal(directorId, goalId, action, { position = null, reason = '' } = {}) {
    ...
    if (!['queued', 'awaiting_owner', 'blocked', 'failed'].includes(goal.status)) {
      throw controlError(`${requestedAction} is allowed only for queued, awaiting_owner, blocked, or failed Goals.`);
    }
    ...
    const workerSafety = goal.status !== 'queued'
      ? await this._assertGoalWorkersStopped(director, goal, { quiescePending: requestedAction === 'cancel' }) : null;
  ...
  async guideGoal(directorId, goalId, { message = '', attachments = [] } = {}) {
    const director = this.getDirector(directorId);
    const goal = this.getGoal(goalId);
    ...
    if (this.goalLocks.has(goal.id)) {
      throw controlError('Goal control is already running; retry guidance after it settles.', 409, 'GOAL_GUIDANCE_BUSY');
    }
    if (director.status === 'running'
      || this.state.runs.some(run => run.goalId === goal.id && run.status === 'running')) {
      throw controlError(
        'A Director checkpoint is currently changing this Goal; retry guidance after that checkpoint settles.',
        409,
        'GOAL_GUIDANCE_BUSY',
      );
    }
    ...
    const previous = {
      ...
      ownerApprovals: cloneJson(goal.ownerApprovals || []),
      pendingAuthorityPlan: cloneJson(goal.pendingAuthorityPlan || null),
      verificationBarrier: cloneJson(goal.verificationBarrier || null),
      finalAudit: cloneJson(goal.finalAudit || null),
      reanalysisRequired: Boolean(goal.reanalysisRequired),
      guidanceReanalysisPending: cloneJson(goal.guidanceReanalysisPending || null),
    ...
    try {
      ...
      const holder = {
        attachments: durableAttachments.slice(-DIRECTOR_ATTACHMENT_LIMITS.maxContextFiles),
      };
      attachmentContext = await this._prepareRunAttachments(director, holder);
      if (!isActiveGoal(goal)) {
        throw controlError('Goal became terminal before guidance could be persisted.', 409, 'GOAL_NOT_ACTIVE');
      }
      if (goal.status === 'awaiting_owner') {
        throw controlError(
          'The Goal began awaiting an exact Owner decision; answer it through the decision control instead.',
          409,
          'GOAL_DECISION_REQUIRED',
        );
      }
      ...
      if (taskIds.length) {
        goal.guidanceReanalysisPending = { guidanceId: guidance.id, afterWave, requestedAt: guidedAt };
      } else {
        goal.guidanceReanalysisPending = null;
        goal.reanalysisRequired = true;
      }
      addGoalEvent(goal, 'owner', 'goal_guidance', 'Owner supplied additional guidance for the active Goal.', {
        guidanceId: guidance.id,
        attachmentCount: storedAttachments.length,
        workerTargetCount: taskIds.length,
      ...
      if (!persisted) {
        ...
        goal.ownerApprovals = previous.ownerApprovals;
        goal.pendingAuthorityPlan = previous.pendingAuthorityPlan;
        goal.verificationBarrier = previous.verificationBarrier;
        goal.finalAudit = previous.finalAudit;
        goal.reanalysisRequired = previous.reanalysisRequired;
        goal.guidanceReanalysisPending = previous.guidanceReanalysisPending;
    ...
    for (const taskId of taskIds) {
      try {
        receipts.push(await this.interveneTask(directorId, taskId, workerMessage));
      } catch (error) {
        errors.push({
          taskId,
  ...
  async answerGoalDecision(directorId, goalId, { answer = '', selectedOption = null } = {}) {
    ...
    if (ownerAction === 'stop') {
      const workerSafety = await this._assertGoalWorkersStopped(director, goal, { quiescePending: true });
      try {
