...
import {
  ...
  WORKFLOWS, isStructuredEvidenceApproved, requiredWorkflowStage, workflowById, workflowPolicyById,
...
function taskBody(goal, run, plan, action, waveIndex) {
  ...
  if (workerKind === 'review') lines.push(
    '',
    '[STRUCTURED TERMINAL RECEIPT]',
    'Your final kanban_complete call MUST pass the complete review.v1 object, with every required key from the loaded review contract, as metadata.report.',
    'Set metadata.review_outcome to the exact string "approved" only for verdict "pass" or "warn"; otherwise set it to "rejected".',
    'Use exactly: kanban_complete(summary=<human summary>, metadata={"report": <complete review.v1 object>, "review_outcome": "approved|rejected"}, artifacts=[]).',
    'Do not rename metadata.report to metadata.review, do not abbreviate the schema, and do not replace pass|warn|fail|inconclusive with approve|reject.',
  );
  if (workerKind === 'gate') lines.push(
    '',
    '[STRUCTURED TERMINAL RECEIPT]',
    'Your final kanban_complete call MUST pass the complete quality-gate.v1 object, with every required key from the loaded quality-gate skill, as metadata.report.',
    ...
    'Do not rename metadata.report, abbreviate the schema, or omit acceptance, reports, blockers, residual_risk, or next_action.',
  );
  lines.push(
    '',
    '[PUBLIC TRACE]',
