...
export class DirectorService extends EventEmitter {
  ...
  async _captureProjectOperationalStatus(director, { excludeRunId = null } = {}) {
    ...
    try {
      boardTasks = await this._refreshBoard(director, { force: true });
      boardFresh = true;
    } catch {
  ...
  submitMessage(directorId, prompt, { mode = 'auto', attachments = [] } = {}) {
    ...
    if (resolvedMode !== 'delegate' && director.status === 'running' && !concurrentStatusQuery) {
      throw new Error('Director is already running');
    }
    ...
    try {
      ...
      if (!concurrentStatusQuery) director.lastRunId = run.id;
      if (!queueGoal && !concurrentStatusQuery) director.status = 'running';
      this._save();
    } catch (error) {
