...
function goalBoardIsolation(tasks, activeGoal) {
  if (!activeGoal) {
    return { safe: true, foreignRunnableTaskIds: [], activeRunnableTaskIds: [] };
  }
  const activeIds = new Set(activeGoal.taskIds || []);
  const runnable = (Array.isArray(tasks) ? tasks : []).filter(task => (
    ['ready', 'todo', 'running'].includes(String(task?.status || '').toLowerCase())
  ));
  const foreignRunnableTaskIds = runnable
  ...
  async tickDirector(directorId, requestedMax = null) {
    ...
    try {
      try {
        ...
        const goalIsolation = goalBoardIsolation(tasks, activeGoalBeforeDispatch);
        ...
        if (shouldDispatch) {
          ...
          dispatchResult = {
            ...
            reconciliationDue, writerSafety: safetyStatus, goalIsolation,
          ...
          dispatchResult = {
            ...
            reconciliationDue: false, dispatchSkipped: true, writerSafety: safetyStatus, goalIsolation,
            fairPeers, fairShare, totalCapacity, dispatchHeldForGoal,
          };
        }
      } finally {
