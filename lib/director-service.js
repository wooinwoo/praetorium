...
function consoleRun(run = {}) {
  ...
  return {
    ...
    output: consoleText(run.output, active ? 1000 : 640),
...
export class DirectorService extends EventEmitter {
  ...
  submitMessage(directorId, prompt, { mode = 'auto' } = {}) {
    ...
    const queueGoal = resolvedMode === 'delegate' && (Boolean(activeGoal) || director.status === 'running');
    if (resolvedMode !== 'delegate' && director.status === 'running') throw new Error('Director is already running');
    if (queueGoal && this._queuedGoals(directorId).length >= MAX_QUEUED_GOALS_PER_DIRECTOR) {
  ...
  _contextualPrompt(run, recoveryNote = '', { stage = 'plan', analysis = null } = {}) {
    ...
    if (!history.length) return [
      liveStatus, communicationContract, catalogPrompt(), stageContract, analysisContext, ownerAnswers, decisionLedger,
  ...
  async _executeChat(runId) {
    ...
    try {
      ...
      this._progress(run, 'directing', plan
  ...
  consoleSummary({ directorId = null } = {}) {
    ...
    return { ...base, revision: sha256Json(base) };
  }
