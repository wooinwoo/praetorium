...
const MUTATION_REQUEST_PATTERNS = Object.freeze([
  ...
  /^\s*(?:(?:please|kindly)\s+|(?:(?:can|could|would|will)\s+you\s+)|(?:i\s+(?:want|need)\s+you\s+to\s+))?(?:build|fix|implement|create|write|edit|delete|remove|refactor|deploy|publish|release|ship|install|run|add|update|change|modify)\b/i,
  /^\s*(?:git\s+(?:push|tag|reset\s+--hard|clean\s+-[a-z]*f)|rm\s+-[a-z]*[rf][a-z]*)\b/i,
]);
const STATUS_QUESTION_PATTERNS = Object.freeze([
  /(?:됐(?:어|나요|니|습니까)|되었(?:어|나요|니|습니까)|했(?:어|나요|니|습니까)|고쳤(?:어|나요|니|습니까)|만들었(?:어|나요|니|습니까)|돌렸(?:어|나요|니|습니까)|끝났(?:어|나요|니|습니까)|완료(?:됐|했)(?:어|나요|니|습니까))\s*[?？!.]*$/i,
  /^\s*(?:did|have|has|was|were|is|are)\b[^?\n]*\b(?:built|fixed|implemented|created|written|edited|deleted|removed|refactored|deployed|published|released|shipped|installed|run|added|updated|changed|modified)\b[^?\n]*[?]\s*$/i,
]);
const OPERATIONAL_QUERY_PATTERNS = Object.freeze([
  /^\s*(?:지금|현재)?\s*(?:작업|태스크|워커|세션|목표|goal|진행(?:\s*상황)?|현황|상태)(?:들|들을|를|은|는|이|가|에는)?[^\n]{0,36}(?:요약|정리|알려|보여|확인|몇\s*개|어디까지|왜\s*안|안\s*바뀌)/i,
  /(?:요약|정리|알려|보여|확인)[^\n]{0,24}(?:작업|태스크|워커|세션|목표|goal|진행|현황|상태)/i,
  /^\s*(?:지금|현재)?\s*(?:뭐|무엇|어떤\s*작업)(?:을|를)?\s*(?:해|하|하는|하고)[^\n]{0,16}[?？!.]*$/i,
  /\b(?:summari[sz]e|show|list|explain|report)\b[^\n]{0,36}\b(?:current\s+)?(?:tasks?|workers?|sessions?|goals?|progress|status)\b/i,
  /\b(?:what|why|how\s+many|where)\b[^\n]{0,36}\b(?:tasks?|workers?|sessions?|goals?|progress|status)\b/i,
]);
const OPERATIONAL_COMPLETION_PATTERNS = Object.freeze([
  /^\s*(?:아직\s*)?(?:(?:이거|이게|이건|그거|그게|그건|저거|저게|저건|(?:이|그|저)\s*(?:작업|일|goal)|(?:작업|일|goal)(?:이|은|는)?)\s*)?(?:아직\s*)?(?:안\s*)?(?:끝났|끝난|끝남|완료됐|완료된|완료|다\s*됐|다\s*됨|종료됐)\s*(?:나|니|냐|어|나요|습니까|는지|거야|건가|인가|임|습니까요)?\s*[?？!.]*$/i,
  /^\s*(?:is|are)\s+(?:it|this|(?:the\s+)?(?:current\s+)?(?:goal|work|task))\s+(?:done|finished|complete)(?:\s+yet)?\s*[?!.]*$/i,
  /^\s*(?:has|have)\s+(?:it|this|(?:the\s+)?(?:current\s+)?(?:goal|work|task))\s+(?:finished|completed)(?:\s+yet)?\s*[?!.]*$/i,
]);
const OPERATIONAL_PROGRESS_PATTERNS = Object.freeze([
  /(?:구현|수정|개발|배포|작업|기능|일)[^\n]{0,28}(?:어디까지|진행\s*(?:상황|상태)?|끝났|끝난|끝남|완료(?:됐|된)?|다\s*됐|다\s*됨)[^\n]{0,12}(?:[?？]|나|니|냐|어|어요|나요|습니까|는지|거야|건가|인가|임)\s*[?？!.]*$/i,
]);
const EXPLICIT_MUTATION_IMPERATIVE_PATTERNS = Object.freeze([
  /(?:고쳐|만들어|옮겨|바꿔|(?:수정|구현|작성|추가|삭제|변경|리팩터|배포|릴리스|실행|설치)(?:해)?)\s*(?:줘|주세요|라|봐|보자|부탁해|부탁해요|부탁드립니다|부탁드려요)?\s*[?？!.]*$/i,
  /(?:^|[.!?]\s*)(?:please\s+)?(?:fix|implement|create|write|edit|delete|remove|refactor|deploy|publish|release|install|run|add|update|change|modify)(?:\s+(?:it|this|that|them))?(?:\s+please)?\s*[.!]*$/i,
]);
const RESULT_LOCATION_PATTERNS = Object.freeze([
  /^\s*(?:결과|최종\s*(?:결과|보고서)|완료\s*보고서|산출물)[^\n]{0,32}(?:어디|받|보|확인|열)/i,
  /(?:현재|진행\s*중인|완료된|이|그|저)\s*(?:goal|목표|작업)[^\n]{0,24}(?:결과|보고서|산출물)[^\n]{0,24}(?:어디|받|보|확인|열)/i,
  /(?:어디|어떻게)[^\n]{0,24}(?:최종\s*(?:결과|보고서)|완료\s*보고서|산출물)/i,
  /(?:주황|오렌지)[^\n]{0,24}(?:불|표시|상태|왜)/i,
  /^\s*where\s+(?:is|can\s+i\s+(?:see|find|open|get))\s+(?:the\s+)?result\s*[?!.]*$/i,
  /\b(?:where|how)\b[^\n]{0,40}\b(?:final\s+(?:result|report)|deliverable)s?\b/i,
]);
const NON_GOAL_OUTPUT_QUERY_PATTERNS = Object.freeze([
  /\b(?:git\s+diff|search|tests?|testing|build|lint|typecheck|type-check|command|query)\b[^\n]{0,36}\b(?:results?|status|output)\b/i,
  /\b(?:results?|status|output)\b[^\n]{0,20}\b(?:of|for|from)\b[^\n]{0,24}\b(?:git\s+diff|search|tests?|testing|build|lint|typecheck|type-check|command|query)\b/i,
]);
...
const EXTERNAL_MUTATION_PATTERNS = Object.freeze([
  ...
  /\b(?:deploy|upload)\b|\bpublish\s+(?:(?:the|an?)\s+)?(?:release|build|version|candidate|package|artifact|site|website|app|application|image|container)\b|\bpublish\b[^\n]{0,40}\b(?:to|on)\s+(?:npm|pypi|registry|production|github|the\s+(?:web|internet))\b|\bship\s+(?:to|the)\b|\brelease\s+(?:the\s+)?(?:build|version|candidate|package|artifact)\b|\bgit\s+tag\b/i,
...
function cleanDecisionImpacts(values, options) {
  if (!Array.isArray(values)) return [];
  const allowed = new Set(options);
  const seen = new Set();
  return values.slice(0, 8).map(value => {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      throw new Error('Owner decision option_impacts must contain objects.');
    }
    const option = String(value.option || '').trim().slice(0, 220);
    const impact = String(value.impact || '').trim().slice(0, 4000);
    if (!option || !impact) throw new Error('Owner decision option impact requires option and impact.');
    if (!allowed.has(option)) throw new Error(`Owner decision impact references unknown option: ${option}.`);
    if (seen.has(option)) throw new Error(`Owner decision impact duplicates option: ${option}.`);
    seen.add(option);
...
export function isOperationalStatusQuery(prompt) {
  const text = String(prompt || '').trim();
  if (!text) return false;
  if (EXPLICIT_MUTATION_IMPERATIVE_PATTERNS.some(pattern => pattern.test(text))) return false;
  if (NON_GOAL_OUTPUT_QUERY_PATTERNS.some(pattern => pattern.test(text))) return false;
  const explicitStatus = OPERATIONAL_COMPLETION_PATTERNS.some(pattern => pattern.test(text))
    || OPERATIONAL_PROGRESS_PATTERNS.some(pattern => pattern.test(text))
    || RESULT_LOCATION_PATTERNS.some(pattern => pattern.test(text));
  if (explicitStatus) return true;
  if (MUTATION_REQUEST_PATTERNS.some(pattern => pattern.test(text))) return false;
  return OPERATIONAL_QUERY_PATTERNS.some(pattern => pattern.test(text));
}

export function inferRequestMode(prompt, requested = 'auto') {
  if (requested === 'conversation' || requested === 'delegate') return requested;
  const text = String(prompt || '').trim();
  const mutationRequested = EXPLICIT_MUTATION_IMPERATIVE_PATTERNS.some(pattern => pattern.test(text))
    || MUTATION_REQUEST_PATTERNS.some(pattern => pattern.test(text));
  if (isOperationalStatusQuery(text)) return 'conversation';
  if (!STATUS_QUESTION_PATTERNS.some(pattern => pattern.test(text)) && mutationRequested) return 'delegate';
  return 'auto';
}
...
export function validateDirectorControl(value, { requiredMode = null, currentWorkflowId = null } = {}) {
  ...
  const ownerDecisionValue = value.owner_decision && typeof value.owner_decision === 'object' ? value.owner_decision : {};
  const ownerDecisionOptions = cleanStrings(ownerDecisionValue.options, 8);
  const ownerDecision = {
    required: Boolean(ownerDecisionValue.required),
    question: ownerDecisionValue.question ? String(ownerDecisionValue.question).trim() : null,
    context: ownerDecisionValue.context ? String(ownerDecisionValue.context).trim().slice(0, 4000) : null,
    recommendation: ownerDecisionValue.recommendation ? String(ownerDecisionValue.recommendation).trim().slice(0, 4000) : null,
    options: ownerDecisionOptions,
    optionImpacts: cleanDecisionImpacts(ownerDecisionValue.option_impacts, ownerDecisionOptions),
    evidence: cleanStrings(ownerDecisionValue.evidence, 16),
  };
  ...
  if (mode === 'delegate') {
    ...
    if (state === 'awaiting_owner' && (!ownerDecision.context || !ownerDecision.recommendation || !ownerDecision.evidence.length)) {
      throw new Error('awaiting_owner state requires decision context, recommendation, and evidence.');
    }
    if (state === 'awaiting_owner' && ownerDecision.options.length
      && ownerDecision.optionImpacts.length !== ownerDecision.options.length) {
      throw new Error('awaiting_owner state requires one option impact for every option.');
    }
...
export const _test = {
  CONTROL_RE, ANALYSIS_RE,
  HIGH_RISK_PATTERNS, EXTERNAL_MUTATION_PATTERNS, MUTATION_REQUEST_PATTERNS, STATUS_QUESTION_PATTERNS,
