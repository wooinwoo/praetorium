...
const OPERATIONAL_COMPLETION_PATTERNS = Object.freeze([
  ...
  /^\s*(?:is|are)\s+(?:it|this|the\s+(?:goal|work|task))\s+(?:done|finished|complete)(?:\s+yet)?\s*[?!.]*$/i,
  /^\s*(?:has|have)\s+(?:it|this|the\s+(?:goal|work|task))\s+(?:finished|completed)(?:\s+yet)?\s*[?!.]*$/i,
]);
...
const RESULT_LOCATION_PATTERNS = Object.freeze([
  /(?:결과|최종\s*보고서|완료\s*보고서|산출물)[^\n]{0,32}(?:어디|받|보|확인|열)/i,
  /(?:어디|어떻게)[^\n]{0,24}(?:결과|최종\s*보고서|완료\s*보고서|산출물)/i,
  /(?:주황|오렌지)[^\n]{0,24}(?:불|표시|상태|왜)/i,
  /\b(?:where|how)\b[^\n]{0,40}\b(?:result|final report|deliverable)s?\b/i,
]);
