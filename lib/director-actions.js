...
export function inferRequestMode(prompt, requested = 'auto') {
  ...
  const mutationRequested = MUTATION_REQUEST_PATTERNS.some(pattern => pattern.test(text));
  if (!mutationRequested && OPERATIONAL_QUERY_PATTERNS.some(pattern => pattern.test(text))) return 'conversation';
