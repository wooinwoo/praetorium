...
function normalizeWorkspaceWriteScope(values, actionId) {
  if (values.some(value => /[*?\[\]{}]/.test(String(value || '')))) {
    throw new Error(`Action ${actionId} workspace_write scope must contain repository-relative literal paths, not glob expressions.`);
  }
  try {
    ...
    throw new Error(`Action ${actionId} workspace_write scope must contain repository-relative literal paths: ${error.message}`);
  }
}
...
export function validateDirectorControl(value, { requiredMode = null, currentWorkflowId = null } = {}) {
  ...
  const actions = rawActions.map((raw, index) => {
    ...
    const writeScope = effect === 'workspace_write'
      ? normalizeWorkspaceWriteScope(rawWriteScope, id)
      : rawWriteScope;
