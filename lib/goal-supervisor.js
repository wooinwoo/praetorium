...
export function buildSupervisionPrompt({ goal, evidence, gateAudit, catalog, reason = 'wave_completed' }) {
  ...
  return [
    ...
    '<PRAETORIUM_CONTROL>',
    '{"schema":"director-action.v1","mode":"delegate","workflow_id":"existing-workflow","state":"executing|awaiting_owner|complete|blocked","requirements":["..."],"decisions":["public operational reason"],"actions":[{"id":"a1","title":"short title","target":"approved-worker-profile","effect":"read_only|workspace_write|external_mutation|skill_activation","task":"bounded worker outcome with relevant evidence","skills":["approved-skill"],"dependencies":[],"write_scope":["repository-relative candidate path or descriptive read-only scope"],"acceptance":["observable evidence"],"wake_on":["completion|finding|failure"]}],"owner_decision":{"required":false,"question":null,"options":[],"evidence":[]}}',
    '</PRAETORIUM_CONTROL>',
    catalog,
