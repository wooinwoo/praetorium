import {
  isStructuredEvidenceApproved, requiredWorkflowStage, WORKER_PROFILES, workflowById,
...
export function buildSupervisionPrompt({ goal, evidence, gateAudit, catalog, reason = 'wave_completed' }) {
  ...
  const requiredTransition = requiredWorkflowStage(goal.workflowId, evidence, {
    gateAudit,
  ...
  return [
    ...
    `Host-required next transition: ${requiredTransition.stage} (${requiredTransition.reason}). The host will reject actions that skip or reverse this transition.`,
    'State rules:',
