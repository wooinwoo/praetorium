...
const CRITERION_STOP_WORDS = new Set([
  'a', 'an', 'and', 'are', 'as', 'at', 'be', 'by', 'complete', 'current', 'each', 'final',
  'for', 'from', 'have', 'in', 'into', 'is', 'it', 'no', 'of', 'on', 'or', 'required', 'the',
  'to', 'was', 'were', 'with',
]);
...
function criterionTokens(value) {
  return new Set((normalizeCriterion(value).normalize('NFKD').match(/[\p{Letter}\p{Number}]+/gu) || [])
    .filter(token => !CRITERION_STOP_WORDS.has(token)));
}

function safeOrderedCriterionMatch(criterion, reportCriterion) {
  const expected = criterionTokens(criterion);
  const reported = criterionTokens(reportCriterion);
  if (expected.size < 3 || reported.size < 3) return false;
  const shared = [...reported].filter(token => expected.has(token)).length;
  if (shared === 0) return false;
  // A shortened subset is safe only when it retains most of the expected
  // criterion. This rejects labels such as "Unauthorized requests" while
  // permitting an ordered descriptive label such as "Required normalization
  // and slug pipeline" for a longer normalization criterion.
  return shared !== reported.size || shared / expected.size >= 0.75;
}
...
export function evaluateGoalAcceptance(goal, evidence = [], { gateTaskId = null } = {}) {
  ...
  const matchedRows = new Array(expectedCriteria.length).fill(null);
  const consumedRows = new Set();
  for (let criterionIndex = 0; criterionIndex < expectedCriteria.length; criterionIndex += 1) {
    const rowIndex = acceptance.findIndex((row, index) => !consumedRows.has(index)
      && normalizeCriterion(row.criterion) === normalizeCriterion(expectedCriteria[criterionIndex]));
    if (rowIndex >= 0) {
      matchedRows[criterionIndex] = acceptance[rowIndex];
      consumedRows.add(rowIndex);
    }
  }
  const exactMatchesStayOrdered = matchedRows.some(Boolean)
    && matchedRows.every((row, index) => !row || acceptance[index] === row);
  if (acceptance.length === expectedCriteria.length && exactMatchesStayOrdered) {
    for (let index = 0; index < expectedCriteria.length; index += 1) {
      if (matchedRows[index] || consumedRows.has(index)) continue;
      if (!safeOrderedCriterionMatch(expectedCriteria[index], acceptance[index]?.criterion)) continue;
      matchedRows[index] = acceptance[index];
      consumedRows.add(index);
    }
  }
