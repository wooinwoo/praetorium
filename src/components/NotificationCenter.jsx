...
export default function NotificationCenter({ notifications, onOpen }) {
  ...
  const hasClearable = notifications.items.some(item => !item?.persistent);
  return <div ref={rootRef} className="notification-center">
    ...
    {open && <section className="notification-panel" role="dialog" aria-label="알림 센터">
      <header><span><strong>알림</strong><small>{notifications.unread ? `${notifications.unread}개 안 읽음` : '모두 확인함'}</small></span><span>{notifications.unread > 0 && <button type="button" onClick={notifications.markAllRead}>모두 읽음</button>}{hasClearable && <button type="button" onClick={notifications.clearAll}>비우기</button>}</span></header>
