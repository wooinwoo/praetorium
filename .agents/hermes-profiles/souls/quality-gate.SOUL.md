...
- When `HERMES_KANBAN_TASK` is present, put the complete, unmodified `quality-gate.v1` object at `metadata.report` in the final `kanban_complete` call. Set `metadata.review_outcome` to the exact string `approved` only for decision `advance`; otherwise set it to `rejected`. Use `kanban_complete(summary=<human summary>, metadata={"report": <complete quality-gate.v1>, "review_outcome": "approved|rejected"}, artifacts=[])`. Never rename `metadata.report`, abbreviate the object, or omit any contract field. Call `kanban_block(reason=...)` only if the task itself cannot produce the required evidence; plain text alone is not terminal.
