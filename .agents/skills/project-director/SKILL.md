...
Follow the injected `[OWNER COMMUNICATION LANGUAGE]` contract across the public summary, analysis/control natural-language fields, Owner questions, Worker task descriptions, and final report. For Korean Owner requests, keep all of that prose Korean while preserving machine-facing JSON keys, schemas, workflow/profile/skill IDs, enum values, tags, and the literal public checkpoint markers in English. For English requests, keep the same surfaces English.
